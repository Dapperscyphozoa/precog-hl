#!/usr/bin/env python3
"""
lsr_service.py — LSR (Liquidity Sweep Reversal) engine.

Runs alongside SMC v2, SMC-LOOSE, Multi-gate, SMC v1 on the same HL account.
Total isolation:
  - Tags all orders with cloid prefix `lsr_`
  - Owns only positions it placed (tracked in /var/data/lsr_state.json)
  - Does NOT touch other engines' state, code, or open positions

STRATEGY — pattern (validated by 50d BT, walk-forward held OOS):

  1. Identify a pool of equal highs/lows on 15m:
     >=3 swing pivots within 0.15% tolerance, all within last ~30h.
  2. A bar SWEEPS the pool: high (or low) takes out the level w/ displacement
     (>= 1.2 × ATR) and elevated volume (>= 1.2 × 20-bar median).
  3. Same bar's body closes BACK INSIDE the pool (rejection):
     bar close in lower 40% of bar's range (short) or upper 40% (long).
  4. Limit entry placed at pool level (fills on retest).
  5. SL just past sweep wick + 0.05% buffer.
  6. TP1 = 1.5R (close 50%, BE-stop), TP2 = 3R (close remaining).

BT economics (cost-adjusted, fees+slippage included):
  Mod-strict (live):  WR 64% net, +0.31R/trade, ~1.9 fires/coin/wk
  Walk-forward OOS:   WR 71% gross, +0.87R gross / +0.31R net

Universe: full HL perp list minus BLACKLIST.

Sizing modes (env LSR_SIZING_MODE):
  'fixed' (default):       LSR_NOTIONAL_USD per trade ($25 default)
  'risk_normalized':       notional = LSR_RISK_USD / risk_pct (variable)
"""
...
# ═══════════════════════════════════════════════════════
# CONFIG — LSR moderate-strict (validated by 7w walk-forward BT)
# ═══════════════════════════════════════════════════════
PARAMS = {
    'pivot_lb': 5,                   # 15m pivot lookback (idx+5 must close to confirm)
    'equal_tol_pct': 0.0015,         # 0.15% tolerance for "equal" pivots
    'equal_lookback': 120,           # 120 × 15m = 30h pool memory
    'min_equal_count': 3,            # require 3+ pivots in cluster
    'sweep_vol': 1.2,                # sweep bar vol > 1.2 × 20-bar median
    'displace': 1.2,                 # sweep bar range > 1.2 × ATR(14)
    'rejection_pct': 0.4,            # close in lower 40% of bar (or upper 40% long)
    'cooldown_bars': 20,             # don't re-fire same level for 20 bars (5h)
    'rr_min': 3.0,                   # TP2 = 3R
    'sl_buf_pct': 0.0005,            # 0.05% buffer past sweep wick
    'timeout_bars': 40,              # 10h post-fill time-stop
    # Live-only: stale-fire guard. Skip if sweep bar is older than this many bars.
    'max_setup_age_bars': 3,         # only fire if sweep bar within last 45 min
}
...
# Sizing
# ═══════════════════════════════════════════════════════
SIZING_MODE = os.environ.get('LSR_SIZING_MODE', 'fixed').strip().lower()
FIXED_NOTIONAL_USD = float(os.environ.get('LSR_NOTIONAL_USD', '25'))
RISK_USD = float(os.environ.get('LSR_RISK_USD', '5'))           # used in risk_normalized
DEFAULT_LEVERAGE = int(os.environ.get('LSR_LEVERAGE', '10'))
MAX_CONCURRENT = int(os.environ.get('LSR_MAX_CONCURRENT', '8'))
COIN_LOSS_COOLDOWN_THRESHOLD = int(os.environ.get('LSR_COIN_LOSS_THRESHOLD', '2'))
COIN_COOLDOWN_HOURS = int(os.environ.get('LSR_COIN_COOLDOWN_HOURS', '24'))
LIVE_TRADING = os.environ.get('LSR_LIVE', '0') == '1'
...
# Storage
STATE_PATH = os.environ.get('LSR_STATE_PATH', '/var/data/lsr_state.json')
...
# ═══════════════════════════════════════════════════════
# LSR ENGINE — Liquidity Sweep Reversal pattern
# ═══════════════════════════════════════════════════════
def lsr_compute_pivots(highs, lows, lb):
    """Return (pivot_highs, pivot_lows) as lists of (idx, value)."""
    n = len(highs); ph = []; pl = []
    for i in range(lb, n - lb):
        if highs[i] == max(highs[i-lb:i+lb+1]): ph.append((i, highs[i]))
        if lows[i]  == min(lows[i-lb:i+lb+1]):  pl.append((i, lows[i]))
    return ph, pl
...
def run_lsr(c15, params, return_armed_only=False):
    """Run LSR detection on a 15m candle series.

    Returns list of setup dicts:
      {is_long, entry, sl, tp1, tp2, sweep_idx, sweep_t, level, pool_count, risk_pct}

    If return_armed_only=True (live mode): only returns the most recent
    sweep within max_setup_age_bars of the current bar, so we don't fire
    on stale historical sweeps.
    """
    n = len(c15)
    if n < 100: return []
    ...
    vols   = [b['v'] for b in c15]

    atr = atr_series(highs, lows, closes, 14)
    pivots_h, pivots_l = lsr_compute_pivots(highs, lows, params['pivot_lb'])

    fills = []
    last_fire = {}  # level → last sweep idx (cooldown across same-level re-fires)
    plb = params['pivot_lb']
    tol = params['equal_tol_pct']
    lookback = params['equal_lookback']
    min_count = params['min_equal_count']

    # Live mode: only scan the last `max_setup_age_bars` candles
    age_limit = params.get('max_setup_age_bars', 3)
    scan_start = max(plb*2 + 5, n - 1 - age_limit) if return_armed_only else plb*2 + 5
    scan_end = n - 1   # don't scan the last bar (still forming)

    for i in range(scan_start, scan_end):
        vbase = _median(vols[max(0, i-20):i]) if i >= 20 else 1.0
        if vbase <= 0 or vols[i] <= params['sweep_vol'] * vbase: continue

        bar_h = highs[i]; bar_l = lows[i]; bar_c = closes[i]; bar_o = opens[i]
        bar_range = bar_h - bar_l
        if bar_range <= 0 or atr[i] <= 0: continue
        if bar_range <= params['displace'] * atr[i]: continue

        # Pivots only confirmed at idx + plb. At bar i, only trust pivots up to i-plb.
        confirmed_max = i - plb

        # ─── SHORT — sweep equal HIGHS ───
        rphs = [(idx,v) for idx,v in pivots_h
                if i - lookback <= idx <= confirmed_max and v <= bar_h]
        if len(rphs) >= min_count:
            rphs.sort(key=lambda x: -x[1])
            top_v = rphs[0][1]
            cluster = [(idx,v) for idx,v in rphs if abs(v - top_v)/top_v < tol]
            if len(cluster) >= min_count:
                pool = sum(v for _,v in cluster) / len(cluster)
                if bar_h > pool * (1 + tol*0.5) and bar_c < pool * (1 - tol*0.2):
                    close_pos = (bar_c - bar_l) / bar_range
                    if close_pos < params['rejection_pct']:
                        key = round(pool, 8)
                        if key in last_fire and i - last_fire[key] < params['cooldown_bars']:
                            pass
                        else:
                            entry = pool
                            ...
                            if risk > 0:
                                tp1 = entry - risk * 1.5
                                tp2 = entry - risk * params['rr_min']
                                fills.append({
                                    'is_long': False, 'entry': entry, 'sl': sl,
                                    'tp1': tp1, 'tp2': tp2,
                                    'sweep_idx': i, 'sweep_t': times[i],
                                    'level': pool, 'pool_count': len(cluster),
                                    'risk_pct': risk / entry,
                                })
                                last_fire[key] = i

        # ─── LONG — sweep equal LOWS ───
        rpls = [(idx,v) for idx,v in pivots_l
                if i - lookback <= idx <= confirmed_max and v >= bar_l]
        if len(rpls) >= min_count:
            rpls.sort(key=lambda x: x[1])
            bot_v = rpls[0][1]
            cluster = [(idx,v) for idx,v in rpls if abs(v - bot_v)/bot_v < tol]
            if len(cluster) >= min_count:
                pool = sum(v for _,v in cluster) / len(cluster)
                if bar_l < pool * (1 - tol*0.5) and bar_c > pool * (1 + tol*0.2):
                    close_pos = (bar_c - bar_l) / bar_range
                    if close_pos > (1 - params['rejection_pct']):
                        key = round(pool, 8)
                        if key in last_fire and i - last_fire[key] < params['cooldown_bars']:
                            pass
                        else:
    ...
    if return_armed_only and fills:
        # Return the freshest sweep (largest sweep_idx)
        fills.sort(key=lambda f: -f['sweep_idx'])
        return [fills[0]]
    return fills
    ...
    for i in range(n):
        ...
        if state == 'SWEPT':
            ...
            if mss:
                ...
                # 2026-05-05: SL floor — sweep_wick anchor produces 0.03–0.20%
                # SLs that fire on noise. Floor at LSR_MIN_SL_PCT (default 0.5%).
                # See FIX 1 audit: 63% of trades flushed at near-BE.
                MIN_SL_PCT = float(os.environ.get('LSR_MIN_SL_PCT', '0.005'))
                if setup['is_long']:
...
def make_cloid(coin, suffix):
    ...
    regardless of coin name length. Prefix `lsr_` distinguishes from
    SMC v2 (`lsr_`), multi-gate, and SMC v1 cloids on the shared wallet.
    """
    raw = f'lsr_{coin}_{int(time.time()*1000)}_{suffix}'.encode('utf-8')
...
def compute_notional(setup):
    ...
    'fixed':            constant LSR_NOTIONAL_USD
    'risk_normalized':  notional sized so abs($ risk) ≈ LSR_RISK_USD per trade
...
def fire_setup(coin, setup, state):
    ...
    # Exit policy — set via env LSR_EXIT_POLICY. Default 'tp1_full' (BT winner).
    #   'tp1_full': 100% size exits at TP1 (1.5R). No TP2, no BE-stop. PF 1.61 net.
    #   'split_be': 50/50 split, BE-stop after TP1. Legacy. PF 0.68 net.
    # Re-BT 50 coins / 90d / 420 fills found tp1_full +65R vs split_be -74R net.
    EXIT_POLICY = os.environ.get('LSR_EXIT_POLICY', 'tp1_full').lower()

    if EXIT_POLICY == 'tp1_full':
        # Single-exit policy: 100% at TP1. No TP2 leg, no BE-stop.
        orders = [
            {
        ...
        # Legacy 50/50 split with BE-stop (kept for A/B fallback).
        orders = [
...
def reconcile_positions(state):
    ...
    for fill in fills:
        ...
        # TP1 leg → move SL to BE
            ...
            # Under tp1_full policy: TP1 closes 100% of size. No remainder, no BE move.
            EXIT_POLICY = os.environ.get('LSR_EXIT_POLICY', 'tp1_full').lower()
            if EXIT_POLICY == 'tp1_full':
                ...
                cancel_orphan_legs(coin, pos, 'tp1')  # cancel SL leg
...
def reconcile_phantoms(state):
    ...
    for coin, pos in list(state['positions'].items()):
        ...
        if coin not in hl_active_coins:
            # Partial-fill is real ONLY if HL still shows a position. If we
            # recorded a partial but HL has no open position, the partial was
            # already closed externally — treat as phantom.
...
def scan_for_setups(state, reconcile_fn=None):
    ...
    for ix, coin in enumerate(coins):
        ...
        try:
            ...
            if need_refresh:
                # LSR is single-timeframe (15m only)
                c15 = fetch_candles(coin, '15m', 30)
                if not c15 or len(c15) < 200:
                    n_skip_data += 1
                    continue
                cache = {'15m': c15, 'fetched': now_s}
            ...
            # Run LSR detection in LIVE mode (only fresh sweeps)
            setups = run_lsr(c15, PARAMS, return_armed_only=True)
            if not setups:
                n_no_setup += 1
                continue
            ...
            log(f'  ARMED {coin}: {"LONG" if s["is_long"] else "SHORT"} '
                ...
                f'risk={risk_pct*100:.3f}% pool_n={s.get("pool_count",0)} '
                f'sweep_t={sweep_t}')

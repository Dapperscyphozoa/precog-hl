...
MAX_POSITIONS   = int(os.environ.get('CONFLUENCE_MAX_POSITIONS', '12'))  # 2026-04-28: lowered 25→12 (cluster risk; total cap 22 with System A=10)
...
def _size_and_fire(coin, signal, equity):
    ...
    try:
        _force = float(os.environ.get('FORCE_NOTIONAL_USD', '44'))
        if _force > 0:
            notional_usd = _force
    except Exception:
        pass
...
def _monitor_exits():
    ...
    for coin, pos in to_check:
        try:
            ...
            # 3: profit lock at +1.5% raw — full close. The trail-handoff
            # was buggy (sl_pct semantics inverted: positive value means
            # X% UNDERWATER not X% locked profit). Reverting to hard close
            # at +1.5% until proper trail is implemented with separate
            # trail_target_pct field and matching close logic.
